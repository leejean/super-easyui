;$(function(){
	var Process=function(){
		...
		var startNum=_this.getStart(proDom);
		//对应开始的步骤table显示，其他table隐藏
		from.children("table").hide();
		from.children("table").eq(startNum).show();
		$(".tsh-processList").eq(startNum).addClass("current");
		$(".tsh-processNum").eq(startNum).addClass("current");
		$(".tsh-processPrompt").eq(startNum).addClass("current");
...
//	window["Process"]=Process;
	...
	Process.prototype={
		...
		getStart:function(param){//获取开始步骤
			var startNum=param.attr("data-start")-1;
			return startNum;
		},
